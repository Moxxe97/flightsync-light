...
describe('BackupTab — signed out', () => {
  it('renders the sign-in button and disables backup + restore', () => {
    ...
    expect(screen.getByRole('button', { name: /Back up now/ }).disabled).toBe(true);
    expect(screen.getByRole('button', { name: /Restore from Drive/ }).disabled).toBe(true);
  });
...
describe('BackupTab — signed in', () => {
  ...
  it('shows the connected email and enables backup + restore', () => {
    ...
    expect(screen.getByRole('button', { name: /Restore from Drive/ }).disabled).toBe(false);
  });
...
describe('BackupTab — error state', () => {
  it("shows a reconnect button when the error says 'reconnexion requise'", () => {
    const backupState = { status: 'error', lastBackup: null, log: [{ detail: 'Token expired — reconnection required' }] };
    const onSignIn = vi.fn();
    render(<BackupTab {...baseProps} authUser={{ email: 'pilote@example.com' }} backupState={backupState} onSignIn={onSignIn} />);
    const reconnect = screen.getByRole('button', { name: /Reconnection required/ });
